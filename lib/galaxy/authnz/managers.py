...
class AuthnzManager(object):
    ...
    def can_user_assume_authz(trans, authz_id):
        ...
        if qres is None:
            msg = "A cloud authorization record with the given `authz_id` (`{}`) not found.".format(
                trans.security.encode_id(authz_id))
            log.debug(msg)
            raise exceptions.ObjectNotFound(msg)
        if qres.user_id != trans.user.id:
            msg = "The requested cloud authozation with ID `{}` is not accessible to user with ID " \
