...
class KeycloakAuthnzTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.keycloak_authnz = keycloak_authnz.KeycloakAuthnz('Keycloak', {
            'VERIFY_SSL': True
        }, {
        ...
        self.trans.request.url = "https://localhost:8000/authnz/keycloak/oidc/callback?state={test_state}&code={test_code}".format(test_state=self.test_state, test_code=self.test_code)
    ...
    def mockTrans(self):
        ...
        class Query:
            ...
            provider = None
            keycloak_authnz_token = None

            def filter_by(self, external_user_id=None, provider=None):
                self.external_user_id = external_user_id
                self.provider = provider
                if self.keycloak_authnz_token:
                    return QueryResult([self.keycloak_authnz_token])
                else:
                    return QueryResult()
    ...
    def test_parse_config(self):
        ...
        self.assertEqual(self.keycloak_authnz.config['well_known_oidc_config_uri'], self._get_idp_url())
    ...
    def test_authenticate_sets_env_var_when_localhost_redirect(self):
        ...
        self.keycloak_authnz.authenticate(self.trans)
    ...
    def test_authenticate_does_not_set_env_var_when_https_redirect(self):
        self.assertTrue(self.keycloak_authnz.config['redirect_uri'].startswith("https:"))
        self.assertIsNone(os.environ.get('OAUTHLIB_INSECURE_TRANSPORT', None))
        self.keycloak_authnz.authenticate(self.trans)
        self.assertIsNone(os.environ.get('OAUTHLIB_INSECURE_TRANSPORT', None))
    ...
    def test_callback_galaxy_user_created_when_no_keycloak_authnz_token_exists(self):
        self.trans.set_cookie(value=self.test_state, name=keycloak_authnz.STATE_COOKIE_NAME)
        self.trans.set_cookie(value=self.test_nonce, name=keycloak_authnz.NONCE_COOKIE_NAME)

        self.assertIsNone(
            self.trans.sa_session.query(KeycloakAuthnzToken)
                .filter_by(external_user_id=self.test_user_id,
                           provider=self.keycloak_authnz.config['provider'])
                .one_or_none()
        )
        ...
        self.assertTrue(self._fetch_token_called)
        self.assertTrue(self._get_userinfo_called)
        self.assertEqual(2, len(self.trans.sa_session.items), "Session has new User and new KeycloakAuthnzToken")
        ...
        # Verify added_keycloak_authnz_token
        added_keycloak_authnz_token = self.trans.sa_session.items[1]
        self.assertIsInstance(added_keycloak_authnz_token, KeycloakAuthnzToken)
        self.assertIs(user, added_keycloak_authnz_token.user)
        self.assertEqual(self.test_access_token, added_keycloak_authnz_token.access_token)
        self.assertEqual(self.test_id_token, added_keycloak_authnz_token.id_token)
        self.assertEqual(self.test_refresh_token, added_keycloak_authnz_token.refresh_token)
        expected_expiration_time = datetime.now() + timedelta(seconds=self.test_expires_in)
        expiration_timedelta = expected_expiration_time - added_keycloak_authnz_token.expiration_time
        ...
        refresh_expiration_timedelta = expected_refresh_expiration_time - added_keycloak_authnz_token.refresh_expiration_time
        self.assertTrue(refresh_expiration_timedelta.total_seconds() < 1)
        self.assertEqual(self.keycloak_authnz.config['provider'], added_keycloak_authnz_token.provider)
        self.assertTrue(self.trans.sa_session.flush_called)

    def test_callback_galaxy_user_not_created_when_user_logged_in_and_no_keycloak_authnz_token_exists(self):
        """
        ...
        self.assertTrue(self._fetch_token_called)
        self.assertTrue(self._get_userinfo_called)
        self.assertEqual(1, len(self.trans.sa_session.items), "Session has new KeycloakAuthnzToken")
        # Verify added_keycloak_authnz_token
        added_keycloak_authnz_token = self.trans.sa_session.items[0]
        self.assertIsInstance(added_keycloak_authnz_token, KeycloakAuthnzToken)
        self.assertIs(user, added_keycloak_authnz_token.user)
        self.assertIs(user, self.trans.user)
        self.assertTrue(self.trans.sa_session.flush_called)

    def test_callback_galaxy_user_not_created_when_keycloak_authnz_token_exists(self):
        self.trans.set_cookie(value=self.test_state, name=keycloak_authnz.STATE_COOKIE_NAME)
        self.trans.set_cookie(value=self.test_nonce, name=keycloak_authnz.NONCE_COOKIE_NAME)
        ...
        existing_keycloak_authnz_token = KeycloakAuthnzToken(
        ...
        self.trans.sa_session._query.keycloak_authnz_token = existing_keycloak_authnz_token
        ...
        self.assertEqual(self.keycloak_authnz.config['provider'], self.trans.sa_session._query.provider)
        self.assertEqual(1, len(self.trans.sa_session.items), "Session has updated KeycloakAuthnzToken")
        session_keycloak_authnz_token = self.trans.sa_session.items[0]
        self.assertIsInstance(session_keycloak_authnz_token, KeycloakAuthnzToken)
        self.assertIs(existing_keycloak_authnz_token, session_keycloak_authnz_token, "existing KeycloakAuthnzToken should be updated")
        # Verify both that existing KeycloakAuthnzToken has the correct values and different values than before
        self.assertEqual(self.test_access_token, session_keycloak_authnz_token.access_token)
        self.assertNotEqual(old_access_token, session_keycloak_authnz_token.access_token)
        self.assertEqual(self.test_id_token, session_keycloak_authnz_token.id_token)
        self.assertNotEqual(old_id_token, session_keycloak_authnz_token.id_token)
        self.assertEqual(self.test_refresh_token, session_keycloak_authnz_token.refresh_token)
        self.assertNotEqual(old_refresh_token, session_keycloak_authnz_token.refresh_token)
        expected_expiration_time = datetime.now() + timedelta(seconds=self.test_expires_in)
        expiration_timedelta = expected_expiration_time - session_keycloak_authnz_token.expiration_time
        self.assertTrue(expiration_timedelta.total_seconds() < 1)
        self.assertNotEqual(old_expiration_time, session_keycloak_authnz_token.expiration_time)
        expected_refresh_expiration_time = datetime.now() + timedelta(seconds=self.test_refresh_expires_in)
        refresh_expiration_timedelta = expected_refresh_expiration_time - session_keycloak_authnz_token.refresh_expiration_time
        self.assertTrue(refresh_expiration_timedelta.total_seconds() < 1)
        self.assertNotEqual(old_refresh_expiration_time, session_keycloak_authnz_token.refresh_expiration_time)
        self.assertTrue(self.trans.sa_session.flush_called)

    def test_disconnect(self):
        keycloak_authnz_token = KeycloakAuthnzToken(
            user=User(email=self.test_email, username=self.test_username),
            external_user_id=self.test_user_id,
            provider=self.keycloak_authnz.config['provider'],
            access_token=self.test_access_token,
        ...
        self.trans.user = keycloak_authnz_token.user
        self.trans.user.keycloak_auth = [keycloak_authnz_token]

        success, message, redirect_uri = self.keycloak_authnz.disconnect("Keycloak", self.trans, "/")

        self.assertEqual(1, len(self.trans.sa_session.deleted))
        deleted_token = self.trans.sa_session.deleted[0]
        self.assertIs(keycloak_authnz_token, deleted_token)
        self.assertTrue(self.trans.sa_session.flush_called)
    ...
    def test_disconnect_when_more_than_one_associated_token_for_provider(self):
        ...
        keycloak_authnz_token1 = KeycloakAuthnzToken(
        ...
        keycloak_authnz_token2 = KeycloakAuthnzToken(
            user=self.trans.user,
        ...
        self.trans.user.keycloak_auth = [keycloak_authnz_token1, keycloak_authnz_token2]
